...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, mlp_dim):
        ...
        for _ in range(depth): # 6层transformer块
...
'''
patch大小为 7x7（对于 28x28 图像，这意味着每个图像 4 x 4 = 16 个patch）、10 个可能的目标类别（0 到 9）和 1 个颜色通道（灰度）
...
class ViT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, channels):
        ...
        num_patches = (image_size // patch_size) ** 2 # 划分为16个小块
        patch_dim = channels * patch_size ** 2 # 一个小块有49(7*7)个维度
 
        self.patch_size = patch_size # 划分的小方块（patch）边长
        self.pos_embedding = nn.Parameter(torch.randn(1, num_patches + 1, dim))
        # 相当于被注册为一个参数（位置参数）
        # 生成一个1行1列的向量，该向量中元素是(17,64)的向量
        self.patch_to_embedding = nn.Linear(patch_dim, dim) # 对每一个patch做线性映射
        self.cls_token = nn.Parameter(torch.randn(1, 1, dim)) # 三维矩阵，一行一列64维，这个token是最终用于分类的token
...
torch.manual_seed(42)  # 设置随机数种子（方便复现）
...
transform_mnist = torchvision.transforms.Compose([torchvision.transforms.ToTensor(),
                                                  torchvision.transforms.Normalize((0.1307,), (0.3081,))])  # 图像变换
